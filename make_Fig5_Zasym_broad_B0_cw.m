...
seq_defs.offsets_ppm   = [seq_defs.M0_offset -100:-10 -8:0.1:8 10:100]; % offset vector [ppm]
...
linest={'-','-','-'};
figure('Name','Figure5_Zasym_broad_B0_B1'),
...
% plotting MTRLD
for jj=1:numel(B1)
    t = annotation('textbox','String',sprintf('%.2f µT',B1(jj)),'Position',[0.63 0.83-0.22*jj 0.1 0.1]); t.LineStyle='None';  t.FontSize = 8;
    t = annotation('textbox','String',sprintf('%.2f µT',B1(jj)),'Position',[0.64 0.845-0.03*jj 0.1 0.1]); t.LineStyle='None'; t.FontSize = 8;
    t = annotation('textbox','String',sprintf('%.2f µT',B1(jj)),'Position',[0.64 0.83-0.22*2 0.1 0.1]); t.LineStyle='None'; t.FontSize = 8;
...
ylim([-0.01 0.09]); xlim([-100 100]); 
end
l=legend('show'); l.FontSize = 8; l.Location='northeastoutside';
end
saveas(gcf,'Figure5_Zasym_broad_B0_B1');
%%
